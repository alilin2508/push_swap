...
int		idx_from_endA(int	*tab, int len, int min, int max, int flag, int index)
{
	...
	while (i >= len / 2)
	{
		...
		{
			if ((flag == 4 && (len - i) < index) || flag > 4)
			{
				flag = 4;
				index = i;
			}
		}
		else if (tab[i] >= ((tab[max] / 5) * 4) && tab[i] <= tab[max] && flag == 5 && (len - i) <= index)
		{
				flag = 5;
				index = i;
		}
